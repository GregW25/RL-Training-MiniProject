...
def phase_observation(env: ManagerBasedRlEnv) -> torch.Tensor:
    """
    Returns the current phase of the motion, normalized to [0, 1].
    
    0.0 = Start of episode
    1.0 = End of episode (Time limit reached)
    
    The policy needs this to track the reference trajectory.
    """
    ...
    return torch.clamp(phase, 0.0, 1.0).unsqueeze(-1)